...
def model_structure(model, img_size):
    ...
    print('-' * 142)
    print('The total number of parameters: ' + str(params))
    print('The parameters of Model {}: {:.2f} M'.format(model._get_name(), params * type_size / 1000 / 1000))
    print('The Gflops of {}: {:.2f} G'.format(model._get_name(), (2 * int(macs) * 1e-9)))
    print('-' * 142)

    return params * 1e-6, macs * 1e-9
