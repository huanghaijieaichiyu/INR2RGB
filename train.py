...
def train(self):
    ...
    for epoch in range(self.epochs):
        ...
        for data in pbar:
            ...
            # 图像拼接还原
            fake_img = np.zeros((self.img_size[0], self.img_size[1], 3), dtype=np.float32)
            fake_img[:, :, 0] = x[0][:, :, 0]
            fake_img[:, :, 1:] = 128 * fake.permute(0, 2, 3, 1).detach().cpu().numpy()[0]
            fake_img = 255. * cv2.cvtColor(fake_img, cv2.COLOR_LAB2RGB)
            # print(fake_img)
            # 加入新的评价指标：PSNR,SSIM
            real_pil = img_pil(img[0])
            psn = peak_signal_noise_ratio(np.array(real_pil, dtype=np.float32), fake_img, data_range=255)

            ssim = structural_similarity(cv2.cvtColor(fake_img, cv2.COLOR_RGB2GRAY),
